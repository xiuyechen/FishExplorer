% get all CellXYZ_norm

% get cIX for all fish

% either draw each fish with different color, or each SuperClus with
% different color

% instead of loading from single fish...
function DrawAnat()
isRefAnat = true;
end

function LoadDataForAnatPlot(hfig,isRefAnat,isPopout,cIX_plot,gIX_plot,clrmap)
args = [];

args.CellXYZ = getappdata(hfig,'CellXYZ_norm');
args.anat_yx = getappdata(hfig,'anat_yx_norm');
args.anat_yz = getappdata(hfig,'anat_yz_norm');
args.anat_zx = getappdata(hfig,'anat_zx_norm');

args.isShowFishOutline = getappdata(hfig,'isShowFishOutline');
args.FishOutline = getappdata(hfig,'FishOutline');

args.absIX = getappdata(hfig,'absIX');
if exist('cIX_plot','var'),
    args.cIX = cIX_plot;
else
    args.cIX = getappdata(hfig,'cIX');
end
if exist('gIX_plot','var'),
    args.gIX = gIX_plot;
else
    args.gIX = getappdata(hfig,'gIX');
end
...
if exist('clrmap','var'),
    ...
    args.clrmap = GetColormap(clrmap_name,numK);
end

% anat masks
args.isShowMasks = getappdata(hfig,'isShowMasks');
if isShowMasks,
