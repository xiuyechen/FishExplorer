function [stimset,stim] = StimulusKeyPreprocessing(frame_turn)
...
%% Standardized Stimulus Key ----------------------------------------------------------------------------- MANUAL INPUT!
% Define normalized code, e.g. same stimulus under different raw stim keys
% can be unified
M_range =    [3,1,3,2, 9,10,11,12, 0,13, 3,14,15, 4,16]; % corresponding to
% raw      = [2,3,4,5,12,13,14,15,22,23,30,31,32,99,100] % for Fish 10

% 0 = all black; 1 = black/white; 2 = white/black; 3 = all white; 4 = all gray;
% 5 = gray/black; 6 = white/gray; 7 = black/gray; 8 = gray/white.
% OMR baseline = not moving?? grey??
...
for i = 1:length(M_range_raw),
    stim(st==M_range_raw(i)) = M_range(i);
    for j = 1:nBlocks,
...
ix_singles = find(diff([0,stim])); % index array to map back to raw indices (frame-number)
singles = stim(ix_singles); % sequence of keys without consecutive repeats.
...
for i_ss = 1:length(stimset),
    ...
    nSetReps = length(stimset(i_ss).ij);
    ...
    for i_SetRep = 1:nSetReps,
        ...
        if length(pattern)==1, % exception, e.g. 'Spontaneous' set, single stimlus key held for full duration of set
            ...
            stimset(i_ss).stops(i_SetRep) = stimset(i_ss).rawstops(i_SetRep);
