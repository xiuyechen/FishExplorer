function M_figs = MultiMotorVisuals(hfig,stimcorr,motorcorr,cIX_in,gIX_in,plotIDs,setID,isScaleClrToRange)
...
if ismember(3,plotIDs)
    ...
    I = LoadCurrentFishForAnatPlot(hfig,cIX_out,gIX_out,clrmap,[],opts);
    fig3 = DrawCellsOnAnat(I);
    i_plot = i_plot+1;
    M_figs{i_plot} = fig3;
end
...
if ismember(4,plotIDs)
    ...
    thres_y = 0.5;% 0.1; for fish8 figure
    [A,U_sorted] = sort(motorcorr,'descend');
    i_end = find(A<thres_y,1,'first');
    IX_largemotor = U_sorted(1:i_end);
