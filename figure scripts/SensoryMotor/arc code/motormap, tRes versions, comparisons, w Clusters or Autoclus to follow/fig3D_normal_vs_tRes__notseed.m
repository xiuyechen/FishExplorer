...
saveDir = [];
saveDir{1} = fullfile(outputDir,'motor_map_notseed_nottRes_031317');
saveDir{2} = fullfile(outputDir,'motor_map_notseed_tRes_031317');
    ...
    for i_fish = range_fish
        ...
        for i_numcell = 2,
            ...
            for i_lr = 1:2
                ...
                M_score = Corr_sorted(i_lr,1:lastIX{i_lr})';
...
%                 setappdata(hfig,'M',M_tRes);
                ...
                %%
                thres_low = prctile(M_score,1);
                thres_high = prctile(M_score,99);
                Xrange = [thres_low,thres_high];
                numC = 64;
                gIX = MapXto1Dcolormap(M_score,Xrange,numC);
                clrmap = hot(numC);
            ...
            % Save plot
            if isSaveFig,
                filename = fullfile(saveDir{i_itr}, ['Fish',num2str(i_fish),'_motormap_',num2str(prctcell),'%_each']);
...
topDir = saveDir{1};
bottomDir = saveDir{2};
newDir = fullfile(outputDir,'motormap_notseed_nottRes_TvsB_tRes');
