...
%% run fish
range_fish = GetFishRange('e');%[1:3,5:18];
...
for i_fish = range_fish
    ...
    [cIX_load,gIX_load,~,~,~,M_0] = LoadSingleFishDefault(i_fish,hfig,ClusterIDs);

    [cIX,gIX] = LoadCluster_Direct(i_fish,12,1);
    M = UpdateIndices_Manual(hfig,cIX,gIX);
    eyebehavior = FindClustermeans(gIX,M);
...
%     end
    ...
    [motor_tAvr,motor_tRes] = GetTrialAvrLongTrace(hfig,eyebehavior);
    stimcorr = DiagCorr(Data_tAvr',Data');
    motorcorr = corr(motor_tRes(i_lr,:)',Data');

    xlims = diag(corr(motor_tAvr',eyebehavior'));
    ylims = diag(corr(motor_tRes',eyebehavior'));
...
%     clrmap = MapXYto2Dcolormap(gIX_in,stimcorr,motorcorr,[0,clrX_max],[0.3,clrY_max],grid);
    ...
    %%
    h = figure('Position',[500,0,300,400]); hold on
...
%     plot([-0.3,1],[thres_plot,thres_plot],'r');
    ...
    %% anat, all cells included in A0.5       % really bad plot, discard
...
% M_reg_name = {'bubbleplot_Le_all_passy','multimotor_Le_anat_all','multimotor_Le_anat_all_passy'};
M_reg_name = {'bubbleplot_Re_all_passy','multimotor_Re_anat_all','multimotor_Re_anat_all_passy'};
