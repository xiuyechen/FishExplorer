...
%     isMotorseed = 1; % choice in main GUI. these motor-seeds are cells chosen for their 
%     strongest correlation to the fictive motor traces, for the left and
%     right side each (no forward swim option), therefore size(behavior,1) == 2. 

% we are using regressors pre-made from motor seeds already, just report back
    ...
    name_array_short = {'motor-left','motor-right'};
else % using fictive signals to make motor regressors
    ...
    for j=1:nRegType,
        ...
        regressors(j).name = [name_array{j}];
        % generate regressor in imaging time
        regressors(j).im = gen_reg_im(gc6, t_gc6, t_im, reg);
        ...
        temp(1:s4_) = shffreg;
        shffreg = temp;
        regressors(j).ctrl = gen_reg_im(gc6, t_gc6, t_im, shffreg);
    end
    %     idx = 0;
