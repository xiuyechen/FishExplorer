...
if ~isempty(M_xyz), % draw cells
    for j = 1:size(M_xyz,1),
        ...
        ixs = ixs_0;
        anat_im(ixs) = cmap(clrIX(j),1)*alpha(j) + anat_im(ixs)*(1-alpha(j)); % R
        ixs = ixs_0 + dimv(1)*dimv(2);
        anat_im(ixs) = cmap(clrIX(j),2)*alpha(j) + anat_im(ixs)*(1-alpha(j)); % G
        ixs = ixs_0 + dimv(1)*dimv(2)*2;
        anat_im(ixs) = cmap(clrIX(j),3)*alpha(j) + anat_im(ixs)*(1-alpha(j)); % B
    end
    ...
    ixs_0 = find(maskIX);
    ixs = ixs_0;
    anat_im(ixs) = cmap(1)*alpha(j) + anat_im(ixs)*(1-alpha(j))*(1-white_alpha(j)) + white_alpha(j); % R
    ixs = ixs_0 + dimv(1)*dimv(2);
    anat_im(ixs) = cmap(2)*alpha(j) + anat_im(ixs)*(1-alpha(j))*(1-white_alpha(j)) + white_alpha(j); % G
    ixs = ixs_0 + dimv(1)*dimv(2)*2;
    anat_im(ixs) = cmap(3)*alpha(j) + anat_im(ixs)*(1-alpha(j))*(1-white_alpha(j)) + white_alpha(j); % B
end
