...
while true, % loop through all qualified seeds           
    ...
    while true % grow ROI: expand mask and examine next neighbors
        ...
        % find next closest foxel, using min dist for thres_merge and avr dist
        % for thres_cap
        D = 1-corr(C(find(Mask),:)',C(IX_rest,:)'); %#ok<FNDSB> % distance matrix between foxels already included in ROI and potential foxels
        if isempty(D),
            break;
        end
        D2 = min(D,[],1);
        [a,ix] = min(D2); 
        ix_cand = IX_rest(ix); % candidate foxel
        ...
        % test candidate foxel with thresholds
