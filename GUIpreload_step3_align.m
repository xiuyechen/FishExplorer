...
for i_fish = 10, %:8,    
    ...
    if M_stimset(i_fish)==1, % fish 1-7
        ...
        CRAZ = mean(reshape(CR_dtr(:,IX_all),nCells,period,[]),3); % 'Cell Response Average Zscore'
        ...
        [stimset,stim_full] = StimulusKeyPreprocessing(frame_turn);
        ...
        %% find average
        CRAZ = []; % 'Cell Response Average Zscore'
        stimtypelist = 1:nTypes;
        numcell = size(CR_dtr,1);
        for i = stimtypelist,
            M = circshift(CR_dtr(:,tlists_raw{i}),shift,2);
            CRAZ = horzcat(CRAZ,mean(reshape(M,numcell,periods(i),[]),3));
        end
    ...
    if M_stimset(i_fish)==1, % fish 1-7
        ...
        FcAvr = []; % 'Cell Response Average Zscore'
        stimtypelist = 1:nTypes;
        numcell = size(CR_dtr,1);
        for i = stimtypelist,
    ...
    %% compile CONST
    CONST = [];
    names = {'ave_stack','anat_yx','anat_yz','anat_zx','CInfo','periods','shift','CellResp','CRAZ','dshift','Fc','FcAvr','stim_full','stimset','tlists','datanames'};
    ...
    nParts = round(numel(CONST.CellResp)/42000000);
    t = zeros(1,nParts);
    ix = round(linspace(1,size(CONST.CellResp,1),nParts+1));
    for i = 1:nParts,
        disp(num2str(i));
        eval(['CellResp_' num2str(i) '= CONST.CellResp(ix(i):ix(i+1),:);']);
