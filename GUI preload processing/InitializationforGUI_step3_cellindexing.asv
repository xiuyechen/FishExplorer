...
%% Initialize VAR
% VAR = [];
load(fullfile(save_dir,'VAR_current.mat'),'VAR');
%%
...
%     mat_dir = fullfile(data_dir,'data_full.mat');
    ...
    i_Cluster = 2;
    cIX_abs = setdiff((1:data.numcell_full),data.IX_inval_anat);
    VAR(i_fish).ClusGroup{i_ClusGroup}(i_Cluster).name = 'all cells (ROI''s)';
    VAR(i_fish).ClusGroup{i_ClusGroup}(i_Cluster).cIX_abs = cIX_abs;
    VAR(i_fish).ClusGroup{i_ClusGroup}(i_Cluster).gIX = ones(length(cIX_abs),1);
    VAR(i_fish).ClusGroup{i_ClusGroup}(i_Cluster).numK = 1;
end
...
%%
save(fullfile(save_dir,'VAR_current.mat'),'VAR');
