...
for i_fish = range_fish,
    ...
    CellRespZ_full = h5read(fullfile(save_dir,'TimeSeries_full.h5'),'/CellRespZ');
    toc
...
%     BasicPlotMaps(cIX,gIX,M,CellXYZ,stim_full,anat_yx,anat_yz);
    ...
    % load other sets
    CellResp_full = h5read(fullfile(save_dir,'TimeSeries_full.h5'),'/CellResp');
    CellRespAvr_full = h5read(fullfile(save_dir,'TimeSeries_full.h5'),'/CellRespAvr');
    CellRespAvrZ_full = h5read(fullfile(save_dir,'TimeSeries_full.h5'),'/CellRespAvrZ');
    
    % trim all
    CellResp = CellResp_full(absIX,:);
    CellRespZ = CellRespZ_full(absIX,:);
    CellRespAvr = CellRespAvr_full(absIX,:);
    CellRespAvrZ = CellRespAvrZ_full(absIX,:);
