...
global hback hfwd hclassname hclassmenu hclusgroupmenu hclusmenu hclusname...
    hdatamenu hopID hloadfish hfishnum hstimreg hmotorreg...
    hcentroidreg;
...
uicontrol('Parent',tab{i_tab},'Style','edit',...
    'Position',[grid(i) yrow(i_row) bwidth*n rheight],'String','1',...
    'Callback',@edit_stimrange_Callback);

i=i+n;
n=3; % only centroids (~mean) of clusters shown on left-side plot, the rest is unchanged
uicontrol('Parent',tab{i_tab},'Style','checkbox','String','Show cluster-centroids',...
    'Position',[grid(i) yrow(i_row) bwidth*n rheight],...
...
%     CellRespAvr = getappdata(hfig,'CellRespAvr');    
    ...
    CellRespAvr = [];   
    stimAvr = [];
    for i = 1:length(periods), % = length(tlists)-1
...
% fix fictive: SWITCH LEFT/RIGHT DIRECTION AGAIN
...
if fishset == 1,
    ...
    if isAvr,
        ...
        tIX = cat(1, tlists{stimrange});     
    end
    
    if isRawtime, % this does not work yet!
        tIX = sort(tIX);
    end
end
...
% setappdata(hfig,'dataname',datanames(ID)); % need work
