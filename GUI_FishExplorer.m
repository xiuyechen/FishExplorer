...
function checkbox_isMotorseed_Callback(hObject,~)
hfig = getParentFigure(hObject);
isMotorseed = get(hObject,'Value');
setappdata(hfig,'isMotorseed',isMotorseed);
GetTimeIndexedData(hfig); % to update 'behavior'
RefreshFigure(hfig);
end
...
if ~isempty(str),
    ...
    [cIX,gIX] = SelectClusterRange(cIX,gIX,range);
    UpdateIndices(hfig,cIX,gIX);
    RefreshFigure(hfig);
end
end
...
switch rankID
    ...
    case 4,
        disp('corr');
        [~,D] = FindCentroid(hfig);
        [gIX,rankscore] = SortGroupIXbyScore(D,gIX,numU);
